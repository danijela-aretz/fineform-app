 ...
 */
export async function createSystemFolders(clientEntityId: string, taxYear: number) {
  const folders: any[] = []
  ...
  const fromClientFolder = await prisma.folder.create({
    data: {
      clientId: clientEntityId,
      name: 'From Client',
  ...
  const toClientFolder = await prisma.folder.create({
    data: {
      clientId: clientEntityId,
      name: 'To Client',
      clientVisible: true,
  ...
  const taxReturnsFolder = await prisma.folder.create({
    data: {
      clientId: clientEntityId,
      name: 'Tax Returns',
      clientVisible: true,
 ...
 */
export async function checkFolderVisibility(
  folderId: string,
  userId: string,
  ...
  if (userType === 'STAFF') {
    ...
    if (folder.restrictedAcl) {
      // Check folder ACL
      const aclEntry = await prisma.folderAcl.findUnique({
        where: {
 ...
 */
export async function getTaxReturnsFolder(
  clientEntityId: string,
  taxYear: number
) {
  // Find Tax Returns folder
  const taxReturnsFolder = await prisma.folder.findFirst({
    where: {
      clientId: clientEntityId,
      folderType: 'tax_returns',
      name: 'Tax Returns',
